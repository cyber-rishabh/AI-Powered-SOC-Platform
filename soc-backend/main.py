"""
main.py — SOC Backend Phase 7 (SOAR Playbook Engine)
...
Phase 6 additions:
  [P6-1]  Import enrich_ip from cti_enricher
  [P6-2]  CTI enrichment inserted into _index_alerts() AFTER detection, BEFORE ES index
  [P6-3]  src_ip extracted from alert dict (src_ip or ip_src) for enrichment
  [P6-4]  cti dict attached to alert doc before indexing
  [P6-5]  Optional risk_score boost (+20) when CTI verdict == "CRITICAL"

Phase 7 additions:
  [P7-1]  Import execute_playbook from soar_engine
  [P7-2]  SOAR playbook execution inserted into _index_alerts() AFTER CTI, BEFORE ES index
  [P7-3]  soar dict attached to alert doc before indexing (mock only, no real actions)
"""
...
# [P6-1] Phase 6: CTI Enrichment
from cti_enricher import enrich_ip

# [P7-1] Phase 7: SOAR Playbook Engine
from soar_engine import execute_playbook
...
app = FastAPI(
    title       = "SOC Backend — Phase 7",
    description = "NXLog → FastAPI → Elasticsearch + Detection + CTI + SOAR + Attack Graph + AI Analysis + Chat",
    version     = "10.0.0",
    lifespan    = lifespan,
)
            ...
            await es.indices.create(
                ...
                mappings = {
                    "properties": {
                        ...
                        "cti":          {"type": "object", "dynamic": True},
                        "soar":         {"type": "object", "dynamic": True},
                    }
                }
            )
...
# ═══════════════════════════════════════════════════════════════════════════════
# [P6-2] ALERT INDEXING WITH CTI + SOAR ENRICHMENT
# Pipeline: alert → cti enrichment → soar playbook → ES index
# ═══════════════════════════════════════════════════════════════════════════════
async def _index_alerts(alerts: list[Alert]) -> None:
    ...
    for alert in alerts:
        ...
        # [P6-3] Extract source IP from alert dict
        src_ip = (doc.get("src_ip") or doc.get("ip_src") or "").strip()

        # [P6-4] CTI enrichment — after detection, before ES indexing
        if src_ip:
            try:
                cti = await enrich_ip(src_ip)
                doc["cti"] = cti
                logger.info(
                    "[CTI] ip=%-15s verdict=%-8s abuse=%d country=%s isp=%s",
                    src_ip,
                    cti.get("verdict", "UNKNOWN"),
                    cti.get("abuse_score", 0),
                    cti.get("country", "?"),
                    cti.get("isp", "?"),
                )
                # [P6-5] Optional risk_score boost for CRITICAL verdicts
                if cti.get("verdict") == "CRITICAL" and "risk_score" in doc:
                    doc["risk_score"] = doc["risk_score"] + 20
                    logger.info(
                        "[CTI] risk_score boosted +20 → %d for ip=%s",
                        doc["risk_score"], src_ip,
                    )
            except Exception as cti_err:
                logger.warning("[CTI] Enrichment failed for ip=%s: %r", src_ip, cti_err)

        # [P7-2] SOAR playbook execution — after CTI, before ES index
        try:
            soar = await execute_playbook(doc)
            if soar:
                doc["soar"] = soar
                logger.info(
                    "[SOAR] rule=%-15s playbook='%s' actions=%d",
                    doc.get("rule_id", "?"),
                    soar.get("playbook", "?"),
                    len(soar.get("actions", [])),
                )
        except Exception as soar_err:
            logger.warning("[SOAR] Playbook execution failed: %r", soar_err)
